...
import { VALID_DATASET_IDS } from './validDatasets.js'
...
function getDatasetForDay(dayIndex) {
  ...
  // First filter by the catalog's availability metadata, then intersect with the
  // runtime-validated allowlist (datasets whose live World Bank / REST Countries
  // fetch actually passes the quality gate — generated by scripts/build-atlas-data.js
  // into src/data/validDatasets.js). Falls back to availability-only if the allowlist
  // is empty (e.g. the atlas snapshot hasn't been run yet).
  const availFiltered = availableDatasets.filter(d => d.estimatedAvailability === 'high' || d.estimatedAvailability === 'medium')
  const validSet = new Set(VALID_DATASET_IDS)
  const suitableDatasets = validSet.size > 0
    ? availFiltered.filter(d => validSet.has(d.id))
    : availFiltered
